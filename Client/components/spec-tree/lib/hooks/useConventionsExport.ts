 ...
 * @param frontmatter - The frontmatter configuration
...
export function formatFrontmatter(frontmatter: MDCFrontmatter): string {
  ...
  if (frontmatter.globs && frontmatter.globs.length > 0) {
    lines.push(`globs: [${frontmatter.globs.map((g) => `"${g.replace(/"/g, '\\"')}"`).join(', ')}]`);
  }
  if (frontmatter.alwaysApply !== undefined) {
    lines.push(`alwaysApply: ${frontmatter.alwaysApply}`);
  }
  if (frontmatter.tags && frontmatter.tags.length > 0) {
    lines.push(`tags: [${frontmatter.tags.map((t) => `"${t.replace(/"/g, '\\"')}"`).join(', ')}]`);
  }
 ...
 * @param context - The project context to validate
 ...
 * @returns Combined MDC content string
 */
export function generateCombinedMDC(
  ...
  const projectName = context.name.trim();

  // Frontmatter
  const frontmatter = options.frontmatter || {
    title: `${projectName} Development Rules`,
 ...
 * @param context - The project context
 * @param options - Export options
 * @returns Array of generated MDC files
 */
export function generateSeparateFiles(
  ...
  const baseTitle = context.name.trim();
