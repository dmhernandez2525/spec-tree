...
export function generateTheme({
  ...
  const applyThemeVariables = (isDark: boolean = false) => {
    ...
    // For the blueprint theme, let's add a subtle blueprint pattern as a background overlay.
    if (style === 'blueprint') {
      // The blueprint theme: Adding a subtle blueprint grid pattern using CSS masks or background images.
      ...
      if (!isDark) {
        root.style.setProperty('--blueprint-pattern', patternDataLight);
      } else {
        root.style.setProperty('--blueprint-pattern', patternDataDark);
      }
      ...
      const root = document.documentElement;
      root.style.removeProperty('--blueprint-pattern');
    }
  };
